...
EOT
  ...
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_https : (
  ...
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_https : (
        v.relative_url == null || (length(v.relative_url) > 0)
      )
    ])
    error_message = "must not be empty"
  }
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_https : (
        v.request_body == null || (length(v.request_body) > 0)
      )
    ])
    error_message = "must not be empty"
  }
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_https : (
        v.request_method == null || (length(v.request_method) > 0)
      )
    ])
    error_message = "must not be empty"
  }
  ...
  # --- Unconfirmed validation candidates, derived from azurerm_data_factory_dataset_http's provider source ---
